...
while off != -1:
    ...
    for patch_off in range(minus_offset):
        print("origin: ")
        print(patched_data[data_offset+patch_off])
        patched_data[data_offset+patch_off] = patched_data[data_offset+patch_off]^0x85
        print("became: ")
        print(patched_data[data_offset+patch_off])
    print(pack('I', data_offset))
    #print(data_offset)
    #print(pack('I', enc_off_int))
    off = funcdata.find('mov\tpc', off+1)
    """

k.write(bytearray(patched_data))
f.close()
a.close()
k.close()
